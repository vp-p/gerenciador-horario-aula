...
public class CadastrarSemestre extends Application {
    ...
    public void start(Stage stage) {
        ...
        adicionar.setOnAction(e -> PopupSemestre());
        ...
        Label subtitulo = new Label("A tabela de semestres cadastrados contém informações essenciais para organizar os cursos em níveis de aprendizado.");
        ...
        // Criar a tabela de semestres
        TableView<Semestre> tabela = new TableView<>();

        // Colunas da tabela
        TableColumn<Semestre, String> colId = new TableColumn<>("ID Semestre");
        colId.setCellValueFactory(new PropertyValueFactory<>("id"));

        TableColumn<Semestre, String> colNome = new TableColumn<>("Semestre");
        colNome.setCellValueFactory(new PropertyValueFactory<>("nome"));

        tabela.getColumns().addAll(colId, colNome);
        tabela.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

        ObservableList<Semestre> dados = FXCollections.observableArrayList(
                new Semestre("001", "1(Primeiro)"),
                new Semestre("002", "2(Segundo)"),
                new Semestre("003", "3(Terceiro)"),
                new Semestre("004", "4(Quarto)"),
                new Semestre("005", "5(Quinto)"),
                new Semestre("006", "6(Sexto)")
        );
    ...
    private void PopupSemestre() {

        Stage popup = new Stage();
        popup.setTitle("Adicionar semestre");

        VBox popupLayout = new VBox(10);
        popupLayout.getStyleClass().add("popup-container");

        Label label = new Label("Semestre");
        label.getStyleClass().add("popup-label");
        TextField textField = new TextField();
        textField.setPromptText("Digite o semestre(ex: 1(Primeiro), 2(Segundo)");
        textField.getStyleClass().add("popup-textfield");

        Button cadastroButton = new Button("Cadastrar semestre");
